...
func simpleGetUrlRequestWithErrorHandling(url : URL, completionHandler : @escaping (Data?, URLResponse?, Error?) -> Void)
    {
...
//        let url = URL(string: "https://httpbin.org/get")!
    ...
    let task = session.dataTask(with: url) { data, response, error in
            ...
            guard let response = response as? HTTPURLResponse, (200...299).contains(response.statusCode) else {
                print("CLPushNotifications: Server error!")
 
                return
            }
            
            guard let mime = response.mimeType, mime == "application/json" else {
                print("CLPushNotifications: Wrong MIME type!")
            ...
            do {
                ...
                print("CLPushNotifications: The Response is : ",json)
            } catch {
